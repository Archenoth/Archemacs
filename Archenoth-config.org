...
#+BEGIN_SRC emacs-lisp
  ...
  otherwise."
    (cl-reduce (lambda (acc dir)
                 (or acc (file-exists-p (concat dir "/" filename))))
...
Now, if =Tern= does not exist, we can also use js2's parser, for
things like =js2-refactor=, which allows for advanced automatic
refactoring such as renaming variables and extracting code blocks with
intelligent attention to scope (But it's only local to the current
file as of the time of writing):
#+BEGIN_SRC emacs-lisp
  (unless *tern-exists*
