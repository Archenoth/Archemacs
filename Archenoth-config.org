...
* Setting up
This section is for setting up configuration that does not need any
fancy packages and for configuring the package manager itself before
working with it.
...
be loaded... That will come after the [[Package checker and installer]]
does its thing.
