...
#+BEGIN_SRC emacs-lisp
  ...
  (check-packages 'pp-c-l 'multiple-cursors 'web-mode 'rsense 'robe
   ...
   'tern-auto-complete 'org-plus-contrib 'racer 'flycheck-rust)
#+END_SRC
...
Also, I like the [[https://github.com/emacsfodder/emacs-firecode-theme][Firecode]] theme. It's really nice!
#+BEGIN_SRC emacs-lisp
  (load-theme 'firecode t)
#+END_SRC

As for in-depth styling, I have my own set of basic defaults:
#+BEGIN_SRC emacs-lisp
...
correct settings defined above are chosen.

After that, in "=<<display-buffer-advice>>=", I have created a
function that can act as "=:filter-args=" advice. The idea for the
advice is to stop Emacs from creating new windows all the time in
fullscreen mode, because that is really annoying.
...
screen mode, loads =powerline=, sets the windowed-mode settings, and
binds =<f11>= to the toggle fullscreen function.
...
And finally, stolen from [[http://emacs.stackexchange.com/a/20818/2039][here]], we have a workaround for Evil entering
Normal mode whenever entering the debugger, even if the minor mode is
added to =evil-emacs-state-modes=:
